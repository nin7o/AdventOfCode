...
char mat [MAX][MAX];
int matOfRatio [MAX][MAX];
int numOfRatio [MAX][MAX];
...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < MAX; i++) {
        ...
        if (number != 0) {
            ...
            if (mat[i][numberstart - 1] == '*') {
                isPartNumber = true;
                numOfRatio[i][numberstart - 1]++;
                if (matOfRatio[i][numberstart - 1] == 0) {
                    matOfRatio[i][numberstart - 1] = number;
                } else {
                    matOfRatio[i][numberstart - 1] = matOfRatio[i][numberstart - 1] * number;
                }
            }
            printf("checking after the number : %c\n", mat[i][numberstart + numberlength]);
            if (mat[i][numberstart + numberlength] == '*') {
                isPartNumber = true;
                numOfRatio[i][numberstart + numberlength]++;
                if (matOfRatio[i][numberstart + numberlength] == 0) {
                    matOfRatio[i][numberstart + numberlength] = number;
                } else {
                    matOfRatio[i][numberstart + numberlength] = matOfRatio[i][numberstart + numberlength] * number;
                }
            }
            ...
            for (int k = numberstart - 1; k <= numberstart + numberlength; k++) {
                ...
                if (mat[i - 1][k] == '*') {
                    isPartNumber = true;
                    numOfRatio[i - 1][k]++;
                    if (matOfRatio[i - 1][k] == 0) {
                        matOfRatio[i - 1][k] = number;
                    } else {
                        matOfRatio[i - 1][k] = matOfRatio[i - 1][k] * number;
                    }
                }
                printf("bottom : %c\n", mat[i + 1][k]);
                if (mat[i + 1][k] == '*') {
            ...
            if (isPartNumber) {
                printf(ANSI_COLOR_GREEN "Part number %d is near a GEAR\n" ANSI_COLOR_RESET, number);
            } else {
                printf(ANSI_COLOR_RED "Part number %d is not near a GEAR\n" ANSI_COLOR_RESET, number);
            }
    ...
    for (int i = 0; i < MAX; i++) {
            for (int j = 0; j < MAX; j++){
                if (numOfRatio[i][j] == 2) {
                    printf(ANSI_COLOR_CYAN "Gear in position [%d][%d] has ratio %d\n" ANSI_COLOR_RESET, i, j, matOfRatio[i][j]);
                } else {
                    matOfRatio[i][j] = 0;
                }
            }
        ...
        // sum of the mat of ratio matrix
        
        for (int i = 0; i < MAX; i++) {
            for (int j = 0; j < MAX; j++){
                sumOfRatio += matOfRatio[i][j];
            }
        }

    printf(ANSI_COLOR_CYAN "-----------------[SUM OF RATIOS = %d]-----------------\n" ANSI_COLOR_RESET, sumOfRatio);
